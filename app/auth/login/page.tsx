...
export default function LoginPage() {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      // For demo, just simulate a successful validation but no actual redirect
  ...
  const handleSocialLogin = (provider: 'google' | 'github') => {
    ...
    // TODO: Clerk social login
    // signIn.authenticateWithRedirect({ strategy: `oauth_${provider}`, ... });
  };
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-[#0F172A] relative flex-col justify-between p-12 overflow-hidden">
        ...
        <div className="relative z-10 flex items-center gap-2">
          <div className="w-10 h-10 rounded-xl bg-green-600 flex items-center justify-center shadow-lg shadow-green-900/20">
            <Leaf className="w-6 h-6 fill-white stroke-white" />
          </div>
          <span className="text-2xl font-black tracking-tight text-white">EcoLoop</span>
        </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12 bg-white relative">
        ...
        <div className="lg:hidden absolute top-8 left-8 flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-green-600 flex items-center justify-center">
            <Leaf className="w-5 h-5 fill-white stroke-white" />
          </div>
